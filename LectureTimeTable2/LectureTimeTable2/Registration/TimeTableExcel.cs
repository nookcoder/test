        ...
        public void PrintTimeTable()
        {
            ...
            for (int col = 2; col <= 24; col++)
            {
                for (int row = 1; row <= 16; row++)
                {
                    if (row == 1)
                    {
                        Console.Write(Convert.ToString(data.GetValue(col, row)));
                    }

                    else
                    {
                        if (data.GetValue(col, row) != null)
                        {
                            noHangleCountOfCourseName = printingCourse.FindNoHangle(Convert.ToString(data.GetValue(col, row)));
                            Console.Write(Convert.ToString(data.GetValue(col, row)).PadLeft(20 - data.GetValue(col, row).ToString().Length + noHangleCountOfCourseName), ' ');
                        }
